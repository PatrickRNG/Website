...
![Event Loop](./event_loop_1.png 'Event loop')
...
![Event Loop](./event_loop_2.png 'Event loop complete diagram')
...
_Done:_ Defines if the sequence is finished or not

_Value:_ Any value returned by the iterator
...
The function implements and returns the `next` function, in which when you call it, it will increment the current index, and return the result, which it's the _value_ and _done_ object. When it's the final index, we just return the `done: true`. In this example, I used just the iteration count for the value.
...
_You can also inject a value by passing the value to the next function, like so, `generator.next('new value')`._
